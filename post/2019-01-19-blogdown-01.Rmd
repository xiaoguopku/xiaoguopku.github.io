...
在RStudio的Console中输入`install.packages("blogdown")`来安装blogdown包。
...
content/文件夹中的内容是我们博客的主体，我们今后增加博客，便是在此新建md文件或Rmd文件，该文件夹下的结构是任意的。static/ 文件夹下包含图片、CSS文件等静态资产文件，所有static/ 文件夹下的内容都会被复制到public/ 文件夹下。public/文件夹中的内容便是要部署到GitHub Pages中的文件。

#### 将网站模板修改为自己需要的内容

由于选取的网站主题不同，下面的操作需要根据不同的主题而有所调整。

你会在content/post/文件夹下看到一些md文件，每篇博文对应1个md文件。打开一个md文件，其中内容分为两部分，上半部分上下用三个减号包围，称为YAML块，它起到设置的作用。下半部分是Markdown文本，作为博文的内容。你将其中的内容换成你的博文内容，再根据情况做个性化设置，保存后就可以看到网站内容的变化了。

你也可以用R Markdown （Rmd文件）代替md文件，YAML块可以保持不变，用R Markdown语法代替Markdown即可。二者差异不大，但Rmd文件中，可执行R代码，而不仅仅是作为文本展示出来。

此外，你需要调整config.toml文件中的内容。不同的主题，config.toml中的内容可能差异非常大，具体如何调整，你需要查阅关于该主题的更多的资料。一般来说，该主题的GitHub项目中，会有README文件，其中会有部分相关内容。
  

### 部署到GitHub Pages中

请参考[GitHub Pages](https://pages.github.com/)官方说明。